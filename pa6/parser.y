...
/*Scope parameter defined - (Used in main.h also)*/
std::vector<std::Scope*> SymTabHead;

/*IR_code parameter also defined - (Used in main.h also)*/
std::vector<std::IR_code*> IR_vector;

/*stacks, maps used for labe counter, current mapping and scope tables*/
std::stack<int> label_counter;
std::map<Mark*, int> newMap;
std::map<Mark*, int>* currMap = &newMap;
...
%union{
	...
	std::node_Ast* node_Ast; /*AST Node defined*/
	std::vector <std::node_Ast*>* expr_vector; /*Expression vector defined*/
}
...
%type <node_Ast> primary factor_prefix postfix_expr mulop addop factor expr_prefix expr assign_expr call_expr
%type <expr_vector> expr_list_tail expr_list

%%
program:	TOKEN_PROGRAM id TOKEN_BEGIN{ /*Global Scope declared*/
std::Scope * globalScope = new std::Scope(global_name); /*Scope added*/
SymTabHead.push_back(globalScope); /*Global Scope added to Mark Head Table*/
...
string_decl:	TOKEN_STRING id TOKEN_OP_NE str TOKEN_OP_SEMIC{ /*Structure of string*/
	Mark *newSym = new std::Mark($2, $4, TOKEN_STRING, 0);
...
var_decl:	var_type id_list TOKEN_OP_SEMIC{
	...
	for(int i = $2 -> size() -1; i >= 0; i--){
		...
		//Create new Mark with value set to NULL, type set to var_type and local_counter points 
		//its location in stack
		std::Mark * newSym = new std::Mark((*$2)[i], NULL, $1, local_counter);
		//IR Code Comment for debugging
		cout << ";" <<  *( (*$2)[i] ) << " the local counter: " << local_counter <<endl;
		//Add the Mark to the last scope in the Mark table
		SymTabHead.back() -> insert_record(*( (*$2)[i] ) , newSym);
		func_var_map[*( (*$2)[i] )] = in_function;
		//Define the Mark type for the IR code
		if($1 == TOKEN_INT){
			s_type = "INT_DECL";
		}
...
param_decl:	var_type id{
	// defining new Mark
	std::Mark * newSym = new std::Mark($2, NULL, $1, ++param_counter);
	// updating SymTabHead with new Mark
	SymTabHead.back() -> insert_record(*($2) , newSym);
...
assign_expr:	id TOKEN_OP_NE expr{			/*Set the assigned Expr Node*/
									std::node_Ast * assign_node = new node_Ast();	/*create assign node*/
									...
									//create the id node
									std::node_Ast * id_node = new node_Ast();	/*Create the id node*/
									id_node -> change_node_type(name_value);	/*assign its node type*/
									std::string s = *($1);	/*store its pointer in the string*/

									//id_node -> add_name(*($1));
									//find out the type of the id by looking up the Mark table need to use for loop later
									int temp;
									if (func_var_map[*($1)])	/*if that variable is present or mapped then,*/
									{
										temp = ( (SymTabHead[SymTabHead.size() - 1]->get_tab())[*($1)] -> get_type() );	/*get the type of the Mark*/
										...
										temp = ( (SymTabHead.front()->get_tab())[*($1)] -> get_type() );	/*get the type of the Mark*/
										id_node -> change_int_or_float(temp == TOKEN_INT);	/*assign that token type to id_node*/
...
read_stmt:		TOKEN_READ TOKEN_OP_LP id_list TOKEN_OP_RP TOKEN_OP_SEMIC{	/*print the 3-address read code to vector*/
				for(int i = ($3->size()) - 1; i >= 0; --i){
					...
					if(func_var_map[*( (*$3)[i] )]){	/*need to check the scope use loop later*/
						if((SymTabHead[SymTabHead.size() - 1]->get_tab())[*( (*$3)[i] )] -> get_type() == TOKEN_INT){ /*If Mark type is int*/
							s_type = "READI";																		/*assign it as read int*/
						}
						else if((SymTabHead[SymTabHead.size() - 1]->get_tab())[*( (*$3)[i] )] -> get_type() == TOKEN_FLOAT){	/*If Mark type is float*/
									...
									if ($1 == NULL){
										...
										$$ = $2;
											...
											if($1->get_int_or_float() == $2->get_int_or_float()){
												//Assigns factor to be the right child of the node_Ast of prefix
												$1 -> add_right_child($2);
...
expr_prefix:	expr_prefix factor addop{
											if($1 == NULL){
												//If expr_prefix is NULL, initialise a addop node_Ast's
												...
												if($1->get_int_or_float() == $2->get_int_or_float()){
														...
														if(($1->get_left_node())->get_node_type() == name_value){
															//If left node_Ast of prefix is a name_value type, set operand1 as its name
															s_op1 = ($1->get_left_node())->get_name();
														}
														else{
														...
														if(($1->get_right_node())->get_node_type() == name_value){
															//If right node_Ast of prefix is a name_value type, set operand2 as its name
															s_op2 = ($1->get_right_node())->get_name();
														}
														else{
...
call_expr:		id TOKEN_OP_LP expr_list TOKEN_OP_RP{
				...
				for (int x = 0; x < $3->size(); x++)	/*for all expression in the list*/
				{
					...
					{
						s = (*$3)[x] -> get_name();	/*store its Mark name*/
					}
					else{
				...
				std::node_Ast * caller_node = new node_Ast();		/*Create new object*/
...
expr_list:		expr expr_list_tail{
			...
			|	{std::vector<std::node_Ast*>* temp = new std::vector<std::node_Ast*>; $$ = temp;};;	/*declared new expr vector*/
...
primary:		TOKEN_OP_LP expr TOKEN_OP_RP{$$=$2;}
			|	id{
								std::node_Ast * id_node = new node_Ast();
								...
								if(func_var_map[*($1)] == true){			/*If that arg is mapped then,*/
									...
									id_node -> add_name(s);
									int temp = ( (SymTabHead[SymTabHead.size() - 1]->get_tab())[*($1)] -> get_type() ); /*Get the unused temp Mark variable from stack*/
			...
			|	TOKEN_INTLITERAL{											/*AST node*/
								std::node_Ast * int_node = new node_Ast();
			...
			|	TOKEN_FLOATLITERAL{											/*AST node*/
									std::node_Ast * float_node = new node_Ast();
...
addop:			TOKEN_OP_PLUS{												/*Assign the node as ADD op node*/
								std::node_Ast * op_node = new node_Ast();
...
cond:			expr compop expr{
									...
									if($1->get_int_or_float() == $3->get_int_or_float()){
										...
										if($1->get_node_type() == name_value){
											...
											if (func_var_map[s1])
											{
												// getting stack number from Mark table
										...
										if($3->get_node_type() == name_value){
											...
											if (func_var_map[s2])
											{
												// getting stack number for Mark table
...
init_stmt:		assign_expr{
							...
							if(($1->get_right_node())->get_int_or_float() == ($1->get_left_node())->get_int_or_float()){
								...
								if(($1->get_right_node())->get_int_or_float() == true){
									...
									//If node_Ast for the value being assigned is of name_value type, get it's name
									if(($1->get_right_node())->get_node_type() == name_value){
